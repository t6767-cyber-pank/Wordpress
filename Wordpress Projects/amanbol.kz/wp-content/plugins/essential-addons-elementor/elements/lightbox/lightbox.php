				...
				<?php
					if ( ($settings['eael_lightbox_trigger_type']) == 'eael_lightbox_trigger_button' ) {
						...
						if( 'button' == $settings['trigger_type'] ) {
							...
							if ( !empty( $settings['eael_lightbox_open_btn_icon']) && $settings['eael_lightbox_open_btn_icon_align'] == 'left' ) {
							...
							if ( !empty( $settings['eael_lightbox_open_btn_icon']) && $settings['eael_lightbox_open_btn_icon_align'] == 'right' ) {
							...
							if( !empty($settings['trigger_only_icon']) ) {
								printf( '<i class="eael-trigger-icon eael-modal-popup-link %1$s %2$s" aria-hidden="true"></i>', $settings['trigger_only_icon'], 'eael-modal-popup-link-' . esc_attr( $this->get_id() ) );
							}
						}else if ( 'image' == $settings['trigger_type'] ) {
							$trigger_image = $this->get_settings( 'trigger_only_image' );
							if ( ! empty( $trigger_image['url'] ) ) {
								printf( '<img class="eael-trigger-image eael-modal-popup-link %1$s" src="%2$s">', 'eael-modal-popup-link-' . esc_attr( $this->get_id() ), esc_url( $trigger_image['url'] ) );
							}
						}
					}
				?>
		...
		<div <?php echo $this->get_render_attribute_string('popup-window-container'); ?>>
			...
			<?php
					...
					if ( 'lightbox_type_image' == ($settings['eael_lightbox_type']) ) {
						printf('<img src="%1$s">', esc_url($popup_image['url']));
