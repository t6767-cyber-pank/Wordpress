...
class Widget_Eael_Filterable_Gallery extends Widget_Base {
	...
	protected function gallery_item_store() {
		...
		foreach( $gallery_items as $gallery ) {
			...
			$gallery_store[$counter]['image'] = ($gallery['eael_fg_gallery_img']['id'] ? wp_get_attachment_image_src($gallery['eael_fg_gallery_img']['id'], 'large')[0] : $gallery['eael_fg_gallery_img']['url']);
			$gallery_store[$counter]['image_full'] = ($gallery['eael_fg_gallery_img']['id'] ? wp_get_attachment_image_src($gallery['eael_fg_gallery_img']['id'], 'full')[0] : $gallery['eael_fg_gallery_img']['url']);
	...
	protected function eael_render_fg_buttons($settings, $item) {
		...
		if( ! empty($settings['eael_section_fg_zoom_icon']) && ($item['show_lightbox'] == true) ) {
			$html .='<a href="'.esc_url($item['image_full']).'" class="eael-magnific-link"><i class="'.$settings['eael_section_fg_zoom_icon'].'" aria-hidden="true"></i></a>';
		}
			...
			if($item['controls'] != '') {
				...
				<div class="gallery-grid-item">';
					...
					if( $settings['eael_fg_caption_style'] === 'card'
						...
						$html .= '<a href="'.esc_url($item['image_full']).'" class="eael-magnific-link media-content-wrap">';
					}
